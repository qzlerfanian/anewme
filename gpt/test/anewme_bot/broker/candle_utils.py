...
def normalize_candle(candle: dict, timeframe: str, *, source_index: int | None = None) -> dict:
    ...
    result = dict(candle)
    result.update({"time": open_time, "open_time": open_time, "close_time": close_time,
                   "timeframe": timeframe, "timestamp_semantics": "open_time",
                   "closed_confirmed": candle.get("closed_confirmed", True)})
...
def closed_only(candles: list[dict], timeframe: str, reference_time: datetime) -> list[dict]:
    ...
    return sorted((c for c in normalized if c["closed_confirmed"] and c["close_time"] <= reference),
