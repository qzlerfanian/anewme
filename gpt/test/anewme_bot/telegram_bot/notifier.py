...
def format_analysis_message(result: AnalysisResult) -> str:
    ...
    text = _format_analysis_body(result)
    identity = []
    if result.request_id:
        identity.append(f'Request ID: {result.request_id}')
    if result.analysis_id:
        identity.append(f'Analysis ID: {result.analysis_id}')
    if result.watch_id:
        identity.append(f'Watch ID: {result.watch_id}')
    if result.analysis_phase:
        identity.append(f'Phase: {result.analysis_phase}')
    identity.append(f'Build: {VERSION}')
    return text + '\n\n' + '\n'.join(identity)


def _format_analysis_body(result: AnalysisResult) -> str:
