# نسخه اصلاح‌شده ۲۰۲۶-۰۸-۲۸

آخرین اصلاح: [RR_PRICE_FIX_FA.md](RR_PRICE_FIX_FA.md) — نرمال‌سازی Tick Size/Digits، حداقل RR و تأیید صریح کندل بسته‌شده.

آخرین قابلیت: [SCHEDULED_BATCHES_FA.md](SCHEDULED_BATCHES_FA.md) — شش نوبت ثابت به وقت ایران، چهار نماد با cutoff مشترک، کنترل سلامت داده، ACK جداگانه 4/4، Retry، Circuit breaker و گزارش روزانه.

راهنمای آخرین ارتقا: [OBSERVABILITY_FA.md](OBSERVABILITY_FA.md) — نسخه `3.1.0-observability`، شناسنامه تصمیم، بازپخش آفلاین مرحله پس از پاسخ مدل و `/health`. این راهنما برای نصب این بسته اولویت دارد؛ تنظیمات فعلی زمان و داده حفظ می‌شوند.

**اصلاح جدید واچ: [WATCH_CLOSE_TEST_FA.md](WATCH_CLOSE_TEST_FA.md). شرط Close دقیق، فقط آخرین کندل و پیام تشخیصی هر M5. راهنمای نصب این اصلاح بر راهنمای ارتقای قبلی اولویت دارد.**

**آخرین نسخه: [راهنمای نصب زمان تأییدشده](INSTALL_TIME_VERIFIED_FA.md). این راهنما بر تمام دستورهای ساعت و مهاجرت قدیمی اولویت دارد. داده جدید در data_utc_v2 است.**

**اصلاح جدید زمان: ابتدا [CLOCK_FIX_FA.md](CLOCK_FIX_FA.md) را بخوانید. راهنمای تعویض تک‌فایل hotfix قبلی دیگر برای این بسته معتبر نیست.**

راهنمای معتبر نصب و محدودیت‌ها: [RELEASE_FA.md](RELEASE_FA.md). متن زیر سند تاریخی است؛ در موارد تعارض به راهنمای جدید مراجعه کنید. گزارش عملکرد تخمینی است و اجرای مستقیم روی Python لینوکس پشتیبانی نمی‌شود.

# ANEWME Analysis Bot — سند تاریخی
...
- پس از Trigger، همان سناریوی قبلی برای تأیید نهایی بررسی می‌شود؛ در این مسیر هیچ Watch جدیدی ساخته نمی‌شود.
...
- زمان دقیق Trigger در `triggered_at` ذخیره می‌شود و همان UPDATE اتمیک اجازه Trigger بعد از Expiration را نمی‌دهد.
- هر کندل بسته برای هر Watch با claim دیتابیسی فقط یک‌بار بررسی می‌شود، حتی اگر چند poll یا process هم‌زمان اجرا شوند.
- اگر بررسی نهایی پس از Trigger دوباره WATCH برگرداند، به معنی تأیید نشدن ورود است و همان سناریو با NO_TRADE بسته می‌شود.
- در تحلیل مستقل بعدی، بازسازی همان سناریوی بسته‌شده با تغییر جزئی سطح یا محدوده سرکوب و به `NO_TRADE` تبدیل می‌شود.
- Trigger و Invalidation سطحی فقط با Close کندل کاملاً بسته‌شده فعال می‌شوند؛ عبور لحظه‌ای و Wick هیچ اثری ندارند.
- زمان MT5 به‌عنوان `open_time` تفسیر می‌شود و `close_time` هر M5/M15/H1 صریحاً در UTC محاسبه و گزارش می‌شود؛ کندل در حال تشکیل یا آینده در هیچ Snapshotی وارد نمی‌شود.
- پیش از ثبت Watch، Close آخرین M5 بسته‌شده با سطح ابطال سنجیده می‌شود؛ سناریوی ازقبل‌باطل مستقیم `NO_TRADE` می‌شود.
- کندل تحلیل اولیه به‌عنوان baseline ذخیره می‌شود و مانیتور فقط کندل‌های بعد از ایجاد Watch را بررسی می‌کند.
- گزارش `WATCH_CANDLE_EVIDENCE` شامل زمان باز/بسته‌شدن UTC، OHLC، Bid/Ask، سطح Trigger/Invalidation، شماره منبع کندل و مجازبودن آن است؛ تصمیم فقط از Close استفاده می‌کند.
- تحلیل پس از Trigger دوباره هر سه تایم‌فریم H1/M15/M5 را با همان مسیر تحلیل اولیه دریافت می‌کند؛ تایم‌فریم بدون کندل جدید عیناً از Snapshot والد بازاستفاده و ثبت می‌شود.
...
**راهنمای معتبر این نسخه: [RELEASE_FA.md](RELEASE_FA.md)**

متن پایین، مستند تاریخی نسخه قبلی است و در موارد تعارض معتبر نیست. معماری صف تحلیل، ارسال پیام، بازیابی بعد از توقف و تست‌ها تغییر کرده‌اند. گزارش عملکرد تخمینی است، نه گزارش معاملات واقعی حساب. نصب مستقیم Python لینوکس به MT5 متصل نمی‌شود. ابتدا راهنمای جدید را بخوانید.
