...
class SafetyRegressionTests(unittest.TestCase):
    ...
    def test_minor_rr_difference_uses_calculated_reference(self):
        result = trade_result(reward_risk_ratio=1.80, take_profit=1.10073)
        # 1.73 calculated vs 1.80 API is 0.07R/4.0%: accepted, canonicalized.
        outcome = validate_trade_result(result, snapshot())
        self.assertFalse(any('RR mismatch جدی' in reason for reason in (outcome.reasons or [])))
        self.assertAlmostEqual(result.trade_details.reward_risk_ratio, 1.73, places=2)

    def test_zero_risk_rr_is_invalid(self):
        result = trade_result(stop_loss=1.0990)
        result.trade_details.entry = 1.0990
        self.assertFalse(validate_trade_result(result, snapshot()).is_valid)
