...
class WatchStructuralScenarioTests(unittest.TestCase):
    ...
    def test_21_unconfirmed_boundary_bar_is_not_closed(self):
        candle = normalize_candle({"time": datetime(2026, 8, 28, 9, 30, tzinfo=timezone.utc),
                                   "open": 1, "high": 2, "low": .5, "close": 1,
                                   "closed_confirmed": False}, "M5")
        self.assertEqual(closed_only([candle], "M5", datetime(2026, 8, 28, 9, 35, tzinfo=timezone.utc)), [])
