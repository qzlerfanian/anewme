...
class AIClient:
    ...
    def _call_with_retry(self, messages: list, max_tokens: int, label: str, response_format=None) -> str:
        """Retry transient failures only; never retry refusals or exhausted quota."""
        if not hasattr(self, '_request_local'):  # supports lightweight diagnostic/test construction
            self._request_local = threading.local()
        for attempt in range(1, self.MAX_ATTEMPTS + 1):
            self._request_local.last_attempts = attempt
            ...
            try:
                ...
                if attempt < self.MAX_ATTEMPTS:
                    ...
                    continue
                raise RuntimeError(f"فراخوانی OpenAI برای {label} بعد از {self.MAX_ATTEMPTS} تلاش ناموفق بود") from exc
    ...
    def acknowledge_scheduled_chart(self, chart_paths, metadata, snapshot):
        """One isolated receipt for one symbol package (three closed TF images)."""
        content = [{'type': 'text', 'text':
            'Validate that this one chart package is readable and internally coherent. '
            'Do not produce a trading decision. Confirm identity fields exactly.\n'
            + json.dumps(metadata, ensure_ascii=False, default=str) + '\n'
            + self._format_market_snapshot(snapshot)}]
        content.extend({'type': 'image_url', 'image_url': {'url': self._encode_image_data_url(path)}}
                       for path in chart_paths)
        schema = {'type': 'json_schema', 'json_schema': {'name': 'chart_receipt', 'strict': True,
            'schema': {'type': 'object', 'additionalProperties': False,
                'properties': {
                    'batch_id': {'type': 'string'}, 'chart_id': {'type': 'string'},
                    'symbol': {'type': 'string'}, 'accepted': {'type': 'boolean'},
                    'reason': {'type': 'string'}, 'description': {'type': 'string'}},
                'required': ['batch_id','chart_id','symbol','accepted','reason','description']}}}
        raw = self._call_with_retry([{'role':'system','content':'You validate one isolated market chart package.'},
            {'role':'user','content':content}], 700, 'scheduled-chart-' + metadata['chart_id'], schema)
        receipt = json.loads(raw)
        for key in ('batch_id', 'chart_id', 'symbol'):
            if receipt.get(key) != metadata[key]:
                raise RuntimeError(f'پاسخ تأیید چارت شناسه {key} نادرست داشت.')
        if receipt.get('accepted') is not True or not receipt.get('description','').strip():
            raise RuntimeError('سرویس داده چارت را معتبر ندانست: ' + str(receipt.get('reason','نامشخص')))
        return receipt, int(getattr(self._request_local, 'last_attempts', 1))

    def request_analysis_from_descriptions(self, symbol, snapshot, descriptions):
        rules_text = load_anewme_rules()
        messages = [{'role':'system','content':DECISION_INSTRUCTIONS}, {'role':'user','content':
            f'--- ANEWME RULES (FULL TEXT) ---\n{rules_text}\n\n--- MARKET DATA ---\n{self._format_market_snapshot(snapshot)}'
            f'\n\n--- CONFIRMED ISOLATED CHART DESCRIPTION ---\n{descriptions}'}]
        self.last_chart_descriptions = descriptions
        return self._call_with_retry(messages, 1500, f'تصمیم نهایی زمان‌بندی‌شده {symbol}')

    def probe_service(self):
        schema={'type':'json_schema','json_schema':{'name':'service_probe','strict':True,
            'schema':{'type':'object','additionalProperties':False,
                      'properties':{'ok':{'type':'boolean'}},'required':['ok']}}}
        raw=self._call_with_retry([{'role':'user','content':'Return ok=true. This is a health probe.'}],
                                  20,'analysis-service-health-probe',schema)
