...
from core.audit import audited_finalization
...
class AnalysisService:
    ...
    # ------------------------------------------------------------------
    @symbol_locked
    def run_prepared_analysis(self, symbol, snapshot, chart_description):
        """Final decision for an already frozen and 4/4-confirmed scheduled input."""
        existing = db.get_active_watch_for_symbol(symbol)
        if existing is not None:
            result = AnalysisResult(
                analysis_time=utc_now(), symbol=symbol, status=AnalysisStatus.WATCH,
                direction=Direction(existing['direction']), grade=Grade(existing['grade']),
                reason='واچ فعال قبلی نمایش داده شد؛ تحلیل زمان‌بندی‌شده جدید برای این نماد اجرا نشد.',
                timeframes_checked=[], watch_details=WatchDetails(
                    Direction(existing['direction']), Grade(existing['grade']), 'واچ فعال موجود',
                    existing['trigger_type'], existing['zone_or_level'],
                    json.loads(existing['timeframes_to_recheck'] or '[]'),
                    existing['expiration'], existing['invalidation_condition']))
            result.analysis_id, result.watch_id = existing['parent_analysis_id'], existing['watch_id']
            result.analysis_phase = 'existing_watch'
            return result
        raw = self.ai_client.request_analysis_from_descriptions(symbol, snapshot, chart_description)
        return self._finalize(symbol, raw, snapshot, [], None, chart_description)
    ...
    def run_initial_analysis(self, symbol: str, needs_correlated_symbols: bool = True) -> AnalysisResult:
        ...
        if existing_watch is not None:
            ...
            result.watch_id = existing_watch['watch_id']
            result.analysis_id = existing_watch['parent_analysis_id']
            result.analysis_phase = 'existing_watch'
            return result
        ...
        if not snapshot.market_open:
            ...
            result.analysis_phase = 'initial'
            db.save_analysis(
                analysis_id=result.analysis_id, symbol=symbol, status=result.status.value,
    ...
    def _finalize(
        ...
        result.watch_id = parent_watch_id
        result.analysis_phase = 'post_trigger' if parent_watch else 'initial'
        ...
        if result.status == AnalysisStatus.TRADE:
            ...
            if not outcome.is_valid:
                ...
                minimum_rr_only = (any(str(reason).startswith('MINIMUM_RR_BELOW_MINIMUM') for reason in outcome.reasons)
                                   and not any(not str(reason).startswith('MINIMUM_RR_BELOW_MINIMUM') for reason in outcome.reasons))
                result = AnalysisResult(
                    analysis_time=result.analysis_time,
                    symbol=result.symbol,
                    status=AnalysisStatus.WATCH if minimum_rr_only else AnalysisStatus.NO_TRADE,
                    direction=result.direction,
                    grade=result.grade,
                    reason=("Calculated RR هنوز به حداقل RR استراتژی نرسیده است؛ ستاپ در وضعیت WATCH باقی می‌ماند. "
                            if minimum_rr_only else "نتیجه TRADE توسط کنترل ایمنی رد شد: ") + "؛ ".join(outcome.reasons),
                    timeframes_checked=result.timeframes_checked,
                    raw_ai_text=result.raw_ai_text,
                    trade_details=result.trade_details if minimum_rr_only else None,
                )
            else:
