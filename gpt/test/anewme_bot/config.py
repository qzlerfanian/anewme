...
except ImportError:  # اجازه می‌دهد تست‌های منطق خالص بدون وابستگی اختیاری اجرا شوند
...
@dataclass
class RiskConfig:
    minimum_reward_risk_ratio: float = float(os.getenv('MINIMUM_REWARD_RISK_RATIO', '1.80'))
    """
...
class AppConfig:
    ...
    allow_unrestricted_telegram: bool = field(
        default_factory=lambda: os.getenv("ALLOW_UNRESTRICTED_TELEGRAM", "false").lower() == "true"
    )
    ...
    chart_retention_hours: int = int(os.getenv("CHART_RETENTION_HOURS", "24"))
    scheduled_analysis_enabled: bool = os.getenv('SCHEDULED_ANALYSIS_ENABLED', 'true').lower() == 'true'
    scheduled_start_grace_seconds: int = int(os.getenv('SCHEDULED_START_GRACE_SECONDS', '180'))
    service_failure_threshold: int = int(os.getenv('SERVICE_FAILURE_THRESHOLD', '2'))
    service_recheck_seconds: int = int(os.getenv('SERVICE_RECHECK_SECONDS', '300'))
