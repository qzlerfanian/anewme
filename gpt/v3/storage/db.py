        ...
        for item in duplicate_symbols:
            ...
            for duplicate in rows[1:]:
                conn.execute(
                    ...
                    (datetime.now(timezone.utc).isoformat(), "مهاجرت: Watch فعال تکراری قدیمی بسته شد.", duplicate["watch_id"]),
                )
        conn.execute(
    ...
    with get_connection() as conn:
        conn.execute(
            """INSERT INTO analyses
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                analysis_id, symbol, datetime.now(timezone.utc).isoformat(), status, direction, grade,
...
def log_event(event_type: str, message: str, symbol: str | None = None, watch_id: str | None = None) -> None:
    with get_connection() as conn:
        conn.execute(
            ...
            (datetime.now(timezone.utc).isoformat(), event_type, symbol, watch_id, message),
        )
...
def log_error(context: str, message: str, symbol: str | None = None) -> None:
    with get_connection() as conn:
        conn.execute(
            ...
            (datetime.now(timezone.utc).isoformat(), symbol, context, message),
        )
...
def create_trade_tracking(
    ...
    with get_connection() as conn:
        conn.execute(
            ...
            (
                analysis_id, symbol, direction, order_type, entry, stop_loss, take_profit,
                risk_percent, reward_risk_ratio, expiration, datetime.now(timezone.utc).isoformat(),
            ),
        )
