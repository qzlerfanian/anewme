...
class WatchMonitor:
    ...
    async def _tick(self) -> None:
        ...
        for watch_row in active_watches:
            ...
            except Exception as exc:  # خرابی یک نماد نباید مانیتور بقیه واچ‌ها را متوقف کند.
                logger.exception("بررسی Watch %s/%s ناموفق بود: %s", watch_row["symbol"], watch_row["watch_id"], exc)
                db.log_error("watch_check", str(exc), symbol=watch_row["symbol"])
                db.log_event(
                    "WATCH_CHECK_FAILED",
                    f"دریافت/بررسی داده این واچ ناموفق بود؛ سایر واچ‌ها ادامه یافتند: {exc}",
                    symbol=watch_row["symbol"], watch_id=watch_row["watch_id"],
                )
                continue
            if not triggered:
                continue
