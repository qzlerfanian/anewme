# سند تاریخی — معماری پیش از اصلاحات

مرجع فعلی [RELEASE_FA.md](../RELEASE_FA.md) است. توضیحات این سند درباره مسیر مانیتور، retry و عملکرد واقعی منسوخ شده‌اند.
...
| `close_watch(watch_id, status, reason)` | پایان قطعی Watch فقط با TRIGGERED/INVALIDATED/EXPIRED |
...
```
# سند تاریخی — معماری نسخه قبلی

این سند برای سابقه حفظ شده است. مرجع معماری فعلی و محدودیت‌ها [راهنمای نسخه اصلاح‌شده](../RELEASE_FA.md) است. ادعاهای ارسال فوری، retry پاسخ ردشده، عملکرد واقعی و مسیر قدیمی مانیتور در این سند دیگر معتبر نیستند.
