...
def cleanup_old_charts() -> None:
    """تصاویر قدیمی‌تر از بازه نگهداری را بدون لمس فایل‌های دیگر پاک می‌کند."""
    cutoff = datetime.now(timezone.utc) - timedelta(hours=max(1, config.chart_retention_hours))
    for path in CHART_TMP_DIR.glob("*.png"):
        try:
